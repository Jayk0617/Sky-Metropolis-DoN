...
function App() {
  ...
  const [hasAirport, setHasAirport] = useState(false);
  ...
  // Check airport status whenever grid changes
  useEffect(() => {
    const exists = grid.flat().some(t => t.buildingType === BuildingType.Airport);
    setHasAirport(exists);
  }, [grid]);
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      const nextGrid = gridRef.current.map(row => row.map(tile => {
        if (tile.constructionTimeLeft && tile.constructionTimeLeft > 0) {
          gridUpdated = true;
          return { ...tile, constructionTimeLeft: tile.constructionTimeLeft - 1 };
        }
        return tile;
      }));
      ...
      // 2. Calculate income/pop gen
      let dailyIncome = 0;
      let dailyPopGrowth = 0;
      let buildingCounts: Record<string, number> = {};

      // Use nextGrid for calculations so fully constructed buildings count immediately on the tick they finish
      nextGrid.flat().forEach(tile => {
        if (tile.buildingType !== BuildingType.None) {
          // Only count active buildings (not under construction)
          if (!tile.constructionTimeLeft || tile.constructionTimeLeft <= 0) {
  ...
  const handleTileClick = useCallback((x: number, y: number) => {
    ...
    if (tool === BuildingType.None) {
      if (currentTile.buildingType !== BuildingType.None) {
        ...
        if (currentStats.money >= demolishCost) {
            ...
            // Clear building and construction time
            newGrid[y][x] = { ...currentTile, buildingType: BuildingType.None, constructionTimeLeft: 0 };
            setGrid(newGrid);
    ...
    if (currentTile.buildingType === BuildingType.None) {
      ...
      if (tool === BuildingType.Airport) {
        const alreadyExists = currentGrid.flat().some(t => t.buildingType === BuildingType.Airport);
        if (alreadyExists) {
           addNewsItem({id: Date.now().toString(), text: "City zoning regulations restrict us to one airport.", type: 'neutral'});
           return;
        }
      }
      ...
      if (currentStats.money >= buildingConfig.cost) {
        ...
        // Road builds instantly, buildings take 3 ticks (game days)
        const buildTime = tool === BuildingType.Road ? 0 : 3;
        
        newGrid[y][x] = { 
          ...currentTile, 
          buildingType: tool,
          constructionTimeLeft: buildTime
        };
        setGrid(newGrid);
        // Sound effect here
      } else {

...
const ToolButton: React.FC<{
  ...
  money: number;
  isDisabled?: boolean;
}> = ({ type, isSelected, onClick, money, isDisabled }) => {
  ...
  return (
    <button
      onClick={onClick}
      disabled={(!isBulldoze && !canAfford) || isDisabled}
      className={`
        ...
        ${(!isBulldoze && !canAfford) || isDisabled ? 'opacity-50 cursor-not-allowed grayscale' : 'cursor-pointer'}
      `}
      title={isDisabled ? "Limit reached (Max 1)" : config.description}
    >
          ...
          <div className="flex gap-1 md:gap-2 min-w-max px-1">
            {tools.map((type) => (
              <ToolButton
                ...
                isDisabled={type === BuildingType.Airport && hasAirport}
